            ...
            Some(cf) => {
                ...
                if cf.contains_key(seg_buf) {
                    ...
                    return Err(Box::new(Error::new(
                        ErrorKind::NotFound,
                        format!("{:?} does not exist in scan records!", &currentfile.path),
                    )));
                }
            }
            None => {
                return Err(Box::new(Error::new(
                    ErrorKind::NotFound,
                    format!(
                        "{:?} is not a directory! Cannot get its children!",
                        &currentfile.path
                    ),
                )))
            }
    ...
    pub fn to_file(&self) -> File {
        ...
        if self.children.is_some() {
            ...
            for child in self.children.as_ref().unwrap().values() {
                trimmed_file_children.push(File {
                    path: child.path.clone(),
                    size: child.size,
                    children: child.children.as_ref().map(|_| Vec::new()),
                });
            }
